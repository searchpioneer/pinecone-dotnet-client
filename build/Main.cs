...
const string project = "Pinecone.Grpc";

var doc = XDocument.Load("Directory.Build.props");
var pineconeVersion = doc.Descendants(XName.Get("PineconeVersion", "http://schemas.microsoft.com/developer/msbuild/2003"))
	.First().Value;
...
cmd.SetHandler(async () =>
{
	...
	{
		var protosTagDir = Path.Combine(protosDir, pineconeVersion);
		if (Directory.Exists(protosTagDir) && Directory.EnumerateFileSystemEntries(protosTagDir).Any())
		{
			Console.WriteLine($"Already downloaded protos for {pineconeVersion}");
			return;
		}

		Directory.CreateDirectory(protosTagDir);
		Console.WriteLine($"Downloading protos for tag {pineconeVersion} to {protosTagDir}");
		var url = $"https://api.github.com/repos/pinecone-io/pinecone-client/tarball/refs/tags/{pineconeVersion}";
		var protoFileRegex = new Regex(".*?client_sdk/src/proto/.*?.proto");
		var client = new HttpClient
		{
			DefaultRequestHeaders = { UserAgent = { new ProductInfoHeaderValue("PineconeNet", "1.0.0") } },
		};
		...
		// add csharp namespace if file does not contain one
		foreach (var file in Directory.EnumerateFiles(protosTagDir))
		{
			...
			{
				if (contents[i].StartsWith("syntax") || string.IsNullOrEmpty(contents[i]))
					continue;
			...
			contents.Insert(index,"option csharp_namespace = \"Pinecone.Grpc\";");
